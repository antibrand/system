...
var ajaxWidgets, ajaxPopulateWidgets, quickDraftLoad;
window.wp = window.wp || {};
...
jQuery(document).ready( function($) {
	...
	// Quick draft.
	quickDraftLoad = function() {
		var act = $('#quickpost-action'), t;

		$( '#quick-draft .submit input[type="submit"], #quick-draft .submit input[type="reset"]' ).prop( 'disabled' , false );

		t = $('#quick-draft').submit( function( e ) {
			e.preventDefault();
			$('#dashboard_quick_draft #publishing-action .spinner').show();
			$('#quick-draft .submit input[type="submit"], #quick-draft .submit input[type="reset"]').prop('disabled', true);

			$.post( t.attr( 'action' ), t.serializeArray(), function( data ) {
				// Replace the form, and prepend the published post.
				$('#quick-draft').html( data );
				$('#quick-draft').removeClass('initial-form');
				quickDraftLoad();
				highlightLatestPost();
				$('#title').focus();
			});
		...
		$('#publish').click( function() { act.val( 'post-quick-draft-publish' ); } );
		...
		$('#quick-draft').on( 'click focusin', function() {
	...
	$( '.meta-box-sortables' ).sortable( 'option', 'containment', '#admin-page-wrap' );
