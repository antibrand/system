	...
	if ( false === $res ) {
		...
		if ( is_wp_error( $request ) ) {
			$res = new WP_Error( 'plugins_api_failed',
				__( 'An unexpected error occurred. Something may be wrong with wordpress.org or this server&#8217;s configuration. Try refreshing the page.' ),
...
?>
	<p><?php printf( __( 'Plugins extend and expand functionality. You may automatically install plugins from the <a href="%1$s">WordPress Plugin Directory</a> or update plugins as needed.' ), __( 'https://wordpress.org/plugins/' ) ); ?></p>
 ...
 *
 * @since Previous 2.7.0
 * @since Previous 4.6.0 The `$type_selector` parameter was deprecated.
...
?>
	<form class="search-form search-plugins" method="get">
		...
		<label><span class="screen-reader-text"><?php _e( 'Search Plugins' ); ?></span>
			<input type="search" name="s" value="<?php echo esc_attr( $term ) ?>" class="wp-filter-search" placeholder="<?php esc_attr_e( 'Search plugins&hellip;' ); ?>" />
		</label>
...
function install_plugin_information() {
	...
	// Array of allowed plugin elements & attributes.
	$plugins_allowedtags = [
		'a' => [
			'href'   => [],
			'title'  => [],
			...
			<?php foreach ( $api->ratings as $key => $ratecount ) {
				...
				<div class="counter-container">
					<span class="counter-label">
						<a href="https://wordpress.org/support/plugin/<?php echo $api->slug; ?>/reviews/?filter=<?php echo $key; ?>" target="_blank" aria-label="<?php echo $aria_label; ?>"><?php printf( _n( '%d star', '%d stars', $key ), $key ); ?></a>
					</span>
