...
function edit_user( $user_id = 0 ) {
	...
	if ( isset( $_POST['role'] ) && current_user_can( 'edit_users' ) ) {
		...
		if ( ! empty( $new_role ) && empty( $editable_roles[$new_role] ) ) {
			wp_die( __( 'You are not allowed to give users that role.' ), 403 );
		}
	}
	...
	if ( $update ) {
		...
		if ( isset( $_POST['rich_editing'] ) && 'false' === $_POST['rich_editing'] ) {
			$user->rich_editing = 'false';
		} else {
			$user->rich_editing = 'true';
		}

		if ( isset( $_POST['syntax_highlighting'] ) && 'false' === $_POST['syntax_highlighting'] ) {
		...
		if ( isset( $_POST['admin_color'] ) ) {
			$user->admin_color = sanitize_text_field( $_POST['admin_color'] );
		} else {
			$user->admin_color = 'default';
		}

		if ( isset( $_POST['admin_bar_front'] ) ) {
	...
	if ( isset( $_POST['comment_shortcuts'] ) && 'true' == $_POST['comment_shortcuts'] ) {
		$user->comment_shortcuts = 'true';
	} else {
...
<div class="error default-password-nag">
	<p>
		<strong><?php _e( 'Notice:' ) ?></strong> <?php _e( 'You\'re using the auto-generated password for your account. Would you like to change it?' ) ?>
	</p>
	<p>
		<a href="<?php echo get_edit_profile_url() . '#password'; ?>"><?php _e( 'Yes, take me to my profile page' ) ?></a>
		<?php echo ' | '; ?>
		<a href="?default_password_nag=0" id="default-password-nag-no"><?php _e( 'No thanks, do not remind me again' ) ?></a>
	</p>
</div>
<?php
}
...
function _wp_personal_data_export_page() {
	require_once( ABSPATH . 'wp-admin/partials/content/data-export-personal.php' );
}
...
function _wp_personal_data_removal_page() {
	require_once( ABSPATH . 'wp-admin/partials/content/data-remove-personal.php' );
}
...
function _wp_privacy_hook_requests_page() {
	...
	add_action( 'load-' . $export_personal_data, 'help_export_personal_data' );
	add_action( 'load-' . $remove_personal_data, 'help_remove_personal_data' );
}

/**
 * Help tabs for export personal data page
 *
 * @return void
 */
function help_export_personal_data() {

	// Identify the current screen to load help tabs.
	...
	$screen->add_help_tab( [
		...
		'title'    => __( 'Overview' ),
		'content'  => null,
		'callback' => 'help_export_personal_data_overview'
	] );
	...
	$screen->add_help_tab( [
		...
		'title'    => __( 'Usage' ),
		'content'  => null,
		'callback' => 'help_export_personal_data_usage'
	] );
	...
	$screen->add_help_tab( [
		...
		'title'    => __( 'Download' ),
		'content'  => null,
		'callback' => 'help_export_personal_data_download'
	] );

	// Add a help sidebar.
	$screen->set_help_sidebar(
		help_export_personal_data_sidebar()
	);
}

/**
 * Overview help tab content for export personal data page
 *
 * @return string Returns the tab markup.
 */
function help_export_personal_data_overview() {

	$html = require_once( ABSPATH . 'wp-admin/partials/help/export-personal-data-overview.php' );

	return apply_filters( 'help_export_personal_data_overview', $html );
}

/**
 * Usage help tab content for export personal data page
 *
 * @return string Returns the tab markup.
 */
function help_export_personal_data_usage() {

	$html = require_once( ABSPATH . 'wp-admin/partials/help/export-personal-data-usage.php' );

	return apply_filters( 'help_export_personal_data_usage', $html );
}

/**
 * Download help tab content for export personal data page
 *
 * @return string Returns the tab markup.
 */
function help_export_personal_data_download() {

	$html = require_once( ABSPATH . 'wp-admin/partials/help/export-personal-data-download.php' );

	return apply_filters( 'help_export_personal_data_download', $html );
}

/**
 * Help sidebar for export personal data page
 *
 * @return string Returns the sidebar markup.
 */
function help_export_personal_data_sidebar() {
...
/**
 * Help tabs for remove personal data page
 *
 * @return void
 */
function help_remove_personal_data() {
	...
	$screen->add_help_tab( [
		...
		'title'    => __( 'Overview' ),
		'content'  => null,
		'callback' => 'help_remove_personal_data_overview'
	] );

	// Add a help sidebar.
	$screen->set_help_sidebar(
		help_remove_personal_data_sidebar()
	);
}

/**
 * Info help tab content for remove personal data page
 *
 * @return string Returns the tab markup.
 */
function help_remove_personal_data_overview() {

	$html = require_once( ABSPATH . 'wp-admin/partials/help/remove-personal-data-overview.php' );

	return apply_filters( 'help_remove_personal_data_overview', $html );
}

/**
 * Help sidebar for remove personal data page
 *
 * @return string Returns the sidebar markup.
 */
function help_remove_personal_data_sidebar() {
 ...
 * @since WP 4.9.6
 */
abstract class WP_Privacy_Requests_Table extends AppNamespace\Admin\WP_List_Table {
	...
	protected function get_views() {
		...
		if ( isset( $_REQUEST['filter-status'] ) ) {
			$current_status = sanitize_text_field( $_REQUEST['filter-status'] );
		} else {
			$current_status = '';
		}
	...
	public function process_bulk_action() {
		...
		if ( isset( $_REQUEST['request_id'] ) ) {
			$request_ids = wp_parse_id_list( wp_unslash( $_REQUEST['request_id'] ) );
		} else {
			$request_ids = [];
		}
	...
	public function column_default( $item, $column_name ) {
		...
		if ( in_array( $column_name, [ 'created_timestamp' ], true ) ) {
				...
				echo '<a href="' . esc_url(
					wp_nonce_url(
						add_query_arg(
							...
							admin_url( 'users.php?page=export_personal_data' )
						),
				...
				echo '<a href="' . esc_url(
					wp_nonce_url(
						add_query_arg(
							...
							admin_url( 'users.php?page=remove_personal_data' )
						),
						'bulk-privacy_requests'
					)
				) . '" class="button">' . esc_html__( 'Remove request' ) . '</a>';
