...
function export_wp( $args = [] ) {
	...
	function wxr_term_meta( $term ) {
		...
		foreach ( $termmeta as $meta ) {
			/**
			 * Filters whether to selectively skip term meta used for exports.
			 *
...
<!-- 7. Import each of the posts, pages, comments, categories, etc. -->
...
<channel>
	...
	<wp:wxr_version><?php echo EXPORT_VERSION; ?></wp:wxr_version>
...
<?php	$postmeta = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->postmeta WHERE post_id = %d", $post->ID ) );
		foreach ( $postmeta as $meta ) :
			/**
			 * Filters whether to selectively skip post meta used for exports.
			 *
			 * Returning a truthy value to the filter will skip the current meta
			 * object from being exported.
			 *
...
<?php		$c_meta = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->commentmeta WHERE comment_id = %d", $c->comment_ID ) );
			foreach ( $c_meta as $meta ) :
				/**
				 * Filters whether to selectively skip comment meta used for exports.
				 *
				 * Returning a truthy value to the filter will skip the current meta
				 * object from being exported.
				 *
