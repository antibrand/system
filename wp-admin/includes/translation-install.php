...
function translations_api( $type, $args = null ) {
	include( ABSPATH . APPINC . '/version.php' ); // include an unmodified $wp_version
	...
	if ( false === $res ) {
		...
		/**
		 * Disable translations API URL
		 *
		 * @todo Review this.
		 *
		 * Formerly:
		 * $url = $http_url = 'http://api.wordpress.org/translations/';
		 */
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( ! is_object( $res ) && ! is_array( $res ) ) {
				$res = new WP_Error( 'translations_api_failed',
					sprintf(
						/* translators: %s: support forums URL */
						__( 'An unexpected error occurred. Something may be wrong with or this server&#8217;s configuration.' )
					),
...
function wp_download_language_pack( $download ) {
	...
	require_once ABSPATH . 'app-includes/classes/backend/class-wp-upgrader.php';
...
/**
 * Check if there is access to the filesystem without asking for credentials.
 *
 * @since 4.0.0
 *
