...
<?php elseif ( !$php_compat || !$mysql_compat ) :
	if ( !$mysql_compat && !$php_compat )
		printf( __('You cannot update because the application requires PHP version %1$s or higher and MySQL version %2$s or higher. You are running PHP version %3$s and MySQL version %4$s.'), $required_mysql_version, $php_version, $mysql_version );
	elseif ( !$php_compat )
		printf( __('You cannot update because the application requires PHP version %1$s or higher. You are running version %2$s.'), $php_version );
	elseif ( !$mysql_compat )
		printf( __('You cannot update because the application requires MySQL version %1$s or higher. You are running version %2$s.'), $mysql_version );
?>
<?php else :
switch ( $step ) :
	case 0:
...
<p><?php _e( 'The application has been updated! Before we send you on your way, we have to update your database to the newest version.' ); ?></p>
