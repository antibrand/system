...
$help_overview = apply_filters( 'help_options_discussion_overview', $help_overview );
 ...
 * @since 1.0.0
 */
$set_help_sidebar = apply_filters( 'set_help_sidebar_options_discussion', '' );
		...
		<?php settings_fields( 'discussion' ); ?>
				...
				<tr>
					...
					<td>
						<fieldset>
							...
							<p><label for="blacklist_keys"><?php _e( 'When a comment contains any of these words in its content, name, URL, email, or IP address, it will be put in the trash. One word or IP address per line. It will match inside words, so &#8220;fruit&#8221; will match &#8220;grapefruit&#8221;.' ); ?></label></p>
							<p>
			...
			<table class="form-table">
				...
				<tr class="avatar-settings<?php if ( ! $show_avatars ) echo ' hide-if-js'; ?>">
					...
					<th scope="row"><?php _e( 'Content Audience' ); ?></th>
					...
					<td>
						<fieldset>
							 ...
							 * Content audiences
							 *
							 * Eliminate this or minimally apply a rating system NOT
							 * rooted in mainstream media.
							 */
							$audiences = apply_filters(
								'content_audiences',
								[
									// Translators: Content suitability rating.
									'A' => __( 'All audiences' ),
									// Translators: Content suitability rating.
									'C' => __( 'Child audiences' ),
									// Translators: Content suitability rating.
									'M' => __( 'Mature audiences' )
								]
							);
							foreach ( $audiences as $key => $audience ) :
								$selected = ( get_option( 'avatar_rating' ) == $key ) ? 'checked="checked"' : '';
								echo "\n\t<label><input type='radio' name='avatar_rating' value='" . esc_attr( $key ) . "' $selected /> $audience</label><br />";
							endforeach;
							?>
