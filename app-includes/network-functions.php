 ...
 * @return Network_Site|void The blog object
 */
 ...
 * the way that the application validates new site signups.
 *
 * @since MU (3.0.0)
 *
 * @global wpdb   $wpdb
...
function wpmu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	...
	$from_name = get_site_option( 'site_name' ) == '' ? APP_NAME : esc_html( get_site_option( 'site_name' ) );
...
function install_blog( $blog_id, $blog_title = '' ) {
	...
	if ( $wpdb->get_results( "DESCRIBE {$wpdb->posts}" ) )
		die( '<h1>' . __( 'Already Installed' ) . '</h1><p>' . __( 'You appear to have already installed the application. To reinstall please clear your old database tables first.' ) . '</p></body></html>' );
...
Username: USERNAME
Password: PASSWORD
Log in here: BLOG_URLapp-login.php
...
--The Team @ SITE_NAME' );
	...
	if ( empty( $current_network->site_name ) )
		$current_network->site_name = APP_NAME;
...
function maybe_add_existing_user_to_blog() {
	...
	wp_die( sprintf( __( 'You have been added to this site. Please visit the <a href="%s">homepage</a> or <a href="%s">log in</a> using your username and password.' ), home_url(), admin_url() ), __( 'Success' ), array( 'response' => 200 ) );
}
...
function wp_maybe_update_network_user_counts( $network_id = null ) {
	...
	/** This filter is documented in app-includes/backend/network-functions.php */
