 ...
 *
 * If the locale is not set already, then the APP_LANG constant is used if it is
...
function get_locale() {
	...
	// APP_LANG was defined in app-config.
	if ( defined( 'APP_LANG' ) ) {
		$locale = APP_LANG;
	}
 ...
 * @param string $plugin_rel_path Optional. Relative APP_PLUGINS_PATH to where the .mo file resides.
 *                                Default false.
...
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	...
	if ( false !== $plugin_rel_path ) {
		$path = APP_PLUGINS_PATH . '/' . trim( $plugin_rel_path, '/' );
	} elseif ( false !== $deprecated ) {
		_deprecated_argument( __FUNCTION__, '2.7.0' );
		$path = ABSPATH . trim( $deprecated, '/' );
	} else {
		$path = APP_PLUGINS_PATH;
	}
 ...
 *
 * @param string $domain             Text domain. Unique identifier for retrieving translated strings.
 * @param string $mu_plugin_rel_path Optional. Relative to `APP_EXTENSIONS_PATH` directory in which the .mo
...
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	...
	$path = APP_EXTENSIONS_PATH . '/' . ltrim( $mu_plugin_rel_path, '/' );
