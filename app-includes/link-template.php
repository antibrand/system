...
function get_home_url( $blog_id = null, $path = '', $scheme = null ) {
	...
	if ( ! in_array( $scheme, array( 'http', 'https', 'relative' ) ) ) {
		if ( is_ssl() && ! is_admin() && 'app-login.php' !== $pagenow )
			$scheme = 'https';
		else
 ...
 * (e.g. app-site-header.php or the APP_ADMIN_DIR/ folder) are accessible.
 *
 ...
 * @return string Admin URL link with optional path appended.
 */
function wp_admin_url( $path = '', $scheme = 'admin' ) {
	return get_admin_url( null, $path, $scheme );
}
...
function includes_url( $path = '', $scheme = null ) {
	$url = site_url( '/' . APP_INC_DIR . '/', $scheme );
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url( APP_ADMIN_DIR . '/network/', $scheme );
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url( APP_ADMIN_DIR . '/user/', $scheme );
 ...
 * @since 4.2.0
 *
 * @param mixed $id_or_email The Gravatar to retrieve a URL for. Accepts a user_id, avatar md5 hash,
...
function get_avatar_data( $id_or_email, $args = null ) {
	...
	if ( ( 'mm' ) == $args['default'] ) {
		...
		$url = app_assets_url( 'images/blank.png' );
	}
