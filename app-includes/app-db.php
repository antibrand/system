...
class wpdb {
	...
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		...
		// app-config.php creation will manually connect when ready.
	 ...
	 * 'global' - returns the global tables for the installation, returning network tables only if running network.
	 * 'ms_global' - returns the network global tables, regardless if current installation is network.
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: 1: app-config.php. 2: database host */
			...
			$message .= '<p>' . sprintf(
				...
				__( 'If you&#8217;re unsure what these terms mean you should probably contact your host.' )
			) . "</p>\n";
	 ...
	 *
	 * Called when the application is generating the table scheme.
	 *
