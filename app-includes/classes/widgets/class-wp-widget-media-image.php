...
class WP_Widget_Media_Image extends WP_Widget_Media {
	...
	public function render_media( $instance ) {
		...
		if ( $attachment ) {
			...
			$caption_size = app_get_image_size_from_meta( $instance['size'], wp_get_attachment_metadata( $attachment->ID ) );
	...
	public function render_control_template_scripts() {
		...
		<script type="text/html" id="tmpl-wp-media-widget-image-fields">
			...
			<p class="media-widget-image-link">
				...
				<input id="{{ elementIdPrefix }}linkUrl" type="text" class="link" value="{{ data.link_url }}" placeholder="http://" pattern="((\w+:)?\/\/\w.*|\w+:(?!\/\/$)|\/|\?|#).*">
			</p>
			<# } #>
		</script>
