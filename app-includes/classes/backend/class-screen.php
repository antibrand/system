...
final class Screen {
	...
	public static function add_old_compat_help( $screen, $help ) {
		self::$old_compat_help[ $screen->id ] = $help;
	}
	...
	public function get_screen_reader_content() {
		return $this->screen_reader_content;
	}
	...
	public function get_screen_reader_text( $key ) {
		...
		if ( ! isset( $this->screen_reader_content[ $key ] ) ) {
			return null;
		}

		return $this->screen_reader_content[ $key ];
	}
	...
	public function set_screen_reader_content( $content = [] ) {
		...
		$this->screen_reader_content = $content;
	}
	...
	public function show_screen_options() {
		...
		if ( is_bool( $this->show_screen_options ) ) {
		...
		switch ( $this->base ) {
			...
			case 'widgets':
				...
				$this->screen_settings = '<p><a id="access-on" href="widgets.php?widgets-access=on&_wpnonce=' . urlencode( $nonce ) . '">' . __( 'Enable accessibility mode' ) . '</a><a id="access-off" href="widgets.php?widgets-access=off&_wpnonce=' . urlencode( $nonce ) . '">' . __( 'Disable accessibility mode' ) . "</a></p>\n";
		...
		if ( $this->screen_settings || $this->_options ) {
			$show_screen = true;
		}
	...
	public function render_per_page_options() {
		...
		if ( 'edit_comments_per_page' == $option ) {
			...
			// This filter is documented in APP_ADMIN_DIR/includes/class-wp-comments-list-table.php.
			$per_page = apply_filters( 'comments_per_page', $per_page, $comment_status );

		} elseif ( 'categories_per_page' == $option ) {

			// This filter is documented in APP_ADMIN_DIR/includes/class-wp-terms-list-table.php.
...
<?php
	...
	public function render_screen_reader_content( $key = '', $tag = 'h2' ) {
		...
		echo "<$tag class='screen-reader-text'>" . $this->screen_reader_content[ $key ] . "</$tag>";
	}
}