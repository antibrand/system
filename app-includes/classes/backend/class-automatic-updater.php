...
class WP_Automatic_Updater {
	...
	protected function send_core_update_notification_email( $item ) {
		 ...
		 * was already notified of the same new version, the application won't repeatedly
	...
	public function update( $type, $item ) {
		...
		switch ( $type ) {
			...
			case 'plugin':
				...
				$context  = APP_PLUGINS_PATH; // We don't support custom Plugin directories, or updates for APP_EXTENSIONS_PATH
				break;
			case 'theme':
		...
		switch ( $type ) {
			case 'core':
				$skin->feedback( __( 'Updating to version %s' ), $item->version );
				$item_name = sprintf( __( 'Version %s' ), $item->version );
				break;
			case 'theme':
	...
	protected function send_email( $type, $core_update, $result = null ) {
		...
		switch ( $type ) {
			case 'success' : // We updated.
				/* translators: 1: Site name, 2: version number. */
				$subject = __( '[%1$s] Your site has updated to version %2$s' );
				break;

			case 'fail' :   // We tried to update but couldn't.
			case 'manual' : // We can't update (and made no attempt).
				/* translators: 1: Site name, 2: version number. */
				$subject = __( '[%1$s]version %2$s is available. Please update!' );
				break;
		...
		switch ( $type ) {
			case 'success' :
				$body .= sprintf( __( 'Your site at %1$s has been updated automatically to version %2$s.' ), home_url(), $core_update->current );
				$body .= "\n\n";
				...
				if ( $newer_version_available ) {
					$body .= "\n\n" . sprintf( __( 'Version %s is also now available.' ), $next_user_core_update->current ) . ' ';
			...
			case 'fail' :
			case 'manual' :
				$body .= sprintf( __( 'Please update your site at %1$s to version %2$s.' ), home_url(), $next_user_core_update->current );
			...
			case 'critical' :
				if ( $newer_version_available )
					$body .= sprintf( __( 'Your site at %1$s experienced a critical failure while trying to update to version %2$s.' ), home_url(), $core_update->current );
				else
					$body .= sprintf( __( 'Your site at %1$s experienced a critical failure while trying to update to the latest version, %2$s.' ), home_url(), $core_update->current );
	...
	protected function send_debug_email() {
		...
		if ( isset( $this->update_results['core'] ) ) {
			...
			if ( $result->result && ! is_wp_error( $result->result ) ) {
				$body[] = sprintf( __( 'Successfully updated to %s' ), $result->name );
			} else {
				$body[] = sprintf( __( 'Failed to update to %s' ), $result->name );
				$failures++;
			}
			$body[] = '';
		}
...
This debugging email is sent when you are using a development version." ) );
			$body[] = '';
