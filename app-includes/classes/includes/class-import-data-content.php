<?php
/**
 * Import data content class
 *
 * Imports content from compatible website management systems.
 *
 * @package App_Package
 * @subpackage Includes
 * @since 1.0.0
 ...
 * @access public
 */
class Import_Data_Content extends Import_Data {
	...
	function import_start( $file ) {
		...
		if ( ! is_file( $file ) ) {
			...
			echo __( 'The file does not exist, please try again.' ) . '</p>';
	...
	function import_end() {
		...
		echo '<p>' . __( 'Remember to update the passwords and roles of imported users.' ) . '</p>';
	...
	function handle_upload() {
		...
		if ( $this->version > $this->max_wxr_version ) {
			...
			printf( __( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.' ), esc_html( $import_data['version'] ) );
		...
		// No author information, grab it from the posts.
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					...
					printf( __( 'Failed to import author %s. Their posts will be attributed to the current user.' ), esc_html( $post['post_author'] ) );
	...
	function import_options() {
		$j = 0;
?>
<form action="<?php echo admin_url( 'admin.php?import=content&amp;step=2' ); ?>" method="post">
...
<?php if ( $this->allow_create_users() ) : ?>
	...
	<p><?php printf( __( 'If a new user is created, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.' ), esc_html( get_option( 'default_role' ) ) ); ?></p>
...
<?php if ( $this->allow_fetch_attachments() ) : ?>
	...
	<h3><?php _e( 'Import Attachments' ); ?></h3>
	<p>
...
<?php endif; ?>
	...
	<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit' ); ?>" /></p>
</form>
<?php
	}
	...
	function author_select( $n, $author ) {
		...
		if ( $create_users ) {
			if ( $this->version != '1.0' ) {
				_e( 'or create new user with login name:' );
				$value = '';
			} else {
				_e( 'as a new user:' );
		...
		if ( ! $create_users && $this->version == '1.0' )
			_e( 'assign posts to an existing user:' );
		else
			_e( 'or assign posts to an existing user:' );
		wp_dropdown_users( array( 'name' => "user_map[$n]", 'multi' => true, 'show_option_all' => __( '- Select -' ) ) );
	...
	function get_author_mapping() {
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			// network adds strtolower to sanitize_user. Need to sanitize here to stop breakage in process_posts.
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.' ), esc_html( $this->authors[$old_login]['author_display_name']) );
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import category %s' ), esc_html( $cat['category_nicename']) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG )
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import post tag %s' ), esc_html( $tag['tag_name']) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG )
					echo ': ' . $id->get_error_message();
				echo '<br />';
				continue;
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import %s %s' ), esc_html( $term['term_taxonomy']), esc_html( $term['term_name']) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG )
					echo ': ' . $id->get_error_message();
				echo '<br />';
				continue;
		...
		foreach ( $this->posts as $post ) {
			...
			*/
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				printf( __( '%s &#8220;%s&#8221; already exists.' ), $post_type_object->labels->singular_name, esc_html( $post['post_title']) );
				echo '<br />';
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							printf( __( 'Failed to import %s %s' ), esc_html( $taxonomy), esc_html( $term['name']) );
							if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG )
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			_e( 'Menu item skipped due to missing menu slug' );
			echo '<br />';
			return;
		}
		...
		if ( ! $menu_id ) {
			printf( __( 'Menu item skipped due to invalid menu slug: %s' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
	...
	function header() {
		echo '<div class="wrap">';
		echo '<h2>' . __( 'Import Content', 'content-importer' ) . '</h2>';
 ...
 * @since  1.0.0
 * @access public
 * @global object wp_import New instance of the WP_Import class.
 * @return void
 */
function app_importer_init() {
	...
	register_importer(
		'content',
		'Content',
		__( 'Import posts, pages, comments, custom fields, categories, and tags from a compatible export file.' ),
		[ $GLOBALS['wp_import'], 'dispatch' ]
	);
}
add_action( 'admin_init', 'app_importer_init' );