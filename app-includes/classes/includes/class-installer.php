...
class Installer {
	...
	public function install_package( $args = [] ) {
		 ...
		 *
		 * @since Previous 2.8.0
		 * @since Previous 4.4.0 The $hook_extra parameter became available.
		 ...
		 */
		$protected_directories = array( ABSPATH, APP_VIEWS_PATH, APP_PLUGINS_PATH, APP_VIEWS_PATH . '/themes' );
		...
		if ( $clear_destination ) {
			...
			/**
			 * Stop if we're not clearing the destination folder and something exists there already.
	...
	public function run( $options ) {
		...
		$res = $this->fs_connect( [ APP_VIEWS_PATH, $options['destination'] ] );
		...
		if ( ! $options['is_multi'] ) {
			 ...
			 * @since Previous 3.7.0 Added to WP_Upgrader::run().
			 * @since Previous 4.6.0 `$translations` was added as a possible argument to `$hook_extra`.
	...
	public static function create_lock( $lock_name, $release_timeout = null ) {
		...
		if ( ! $lock_result ) {
			...
			Installer :: release_lock( $lock_name );

			return Installer :: create_lock( $lock_name, $release_timeout );
		}
