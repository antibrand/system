 ...
 * @since 3.3.0
 *
 * Private, not included by default. See wp_editor() in app-includes/general-template.php.
 */
...
final class _WP_Editors {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$wrap_class = 'app-core-ui wp-editor-wrap ' . $switch_class;
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * should be added to the TinyMCE instance.
					 *
					 * @since 3.3.0
					 *
					...
					if ( ! empty( $mce_external_plugins ) ) {
						...
						foreach ( $mce_external_plugins as $name => $url ) {
							...
							if ( ! in_array( $name, $loaded_langs, true ) ) {
								...
								$path = APP_VIEWS_PATH . $path . '/langs/';
	...
	public static function wp_link_dialog() {
		...
		<div id="wp-link-wrap" class="app-core-ui" style="display: none" role="dialog" aria-labelledby="link-modal-title">
