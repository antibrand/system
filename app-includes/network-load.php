<?php
/**
 * These functions are needed to load network.
 *
 * @since 3.0.0
 *
 * @package App_Package
 ...
 *
 * The default directory is APP_PLUGINS_DIR. To change the default directory
 * manually, define `APP_PLUGINS_PATH` and `APP_PLUGIN_URL` in the configuration file.
 *
 * @access private
 * @since 3.1.0
 *
 ...
 * use the APP_VIEWS_DIR/blog-deleted.php, blog-inactive.php and
...
function ms_site_check() {
	...
	*/
	...
	if ( '1' == $blog->deleted ) {
		if ( file_exists( APP_VIEWS_PATH . '/blog-deleted.php' ) )
			return APP_VIEWS_PATH . '/blog-deleted.php';
		else
			wp_die( __( 'This site is no longer available.' ), '', array( 'response' => 410 ) );
	}

	if ( '2' == $blog->deleted ) {
		if ( file_exists( APP_VIEWS_PATH . '/blog-inactive.php' ) ) {
			return APP_VIEWS_PATH . '/blog-inactive.php';
		} else {
	...
	if ( $blog->archived == '1' || $blog->spam == '1' ) {
		if ( file_exists( APP_VIEWS_PATH . '/blog-suspended.php' ) )
			return APP_VIEWS_PATH . '/blog-suspended.php';
		else
...
function get_network_by_path( $domain, $path, $segments = null ) {
	return AppNamespace\Network\Network::get_by_path( $domain, $path, $segments );
}
...
function ms_load_current_site_and_network( $domain, $path, $subdomain = false ) {
	...
	// If the network is defined in the configuration file, we can simply use that.
	if ( defined( 'DOMAIN_CURRENT_SITE' ) && defined( 'PATH_CURRENT_SITE' ) ) {
		...
		if ( empty( $current_site ) ) {
			$current_site = AppNamespace\Network\Network::get_by_path( $domain, $path, 1 );
		}
		...
		$current_blog = get_site_by_path( $domain, $path, 1 );
		if ( $current_blog ) {
			$current_site = AppNamespace\Network\Network::get_instance( $current_blog->site_id ? $current_blog->site_id : 1 );
		} else {
	...
	if ( $current_blog && $current_blog->site_id != $current_site->id ) {
		$current_site = AppNamespace\Network\Network::get_instance( $current_blog->site_id );
	}

	// No network has been found, bail.
	if ( empty( $current_site ) ) {
		/** This action is documented in wp-includes/network-settings.php */
		do_action( 'ms_network_not_found', $domain, $path );
	...
	if ( empty( $current_blog ) ) {
		...
		if ( $subdomain && ! defined( 'NOBLOGREDIRECT' ) ) {
			...
			$destination .= 'app-signup.php?new=' . str_replace( '.' . $current_site->domain, '', $domain );
		} elseif ( $subdomain ) {
...
function ms_not_installed( $domain, $path ) {
	...
	if ( ! $wpdb->get_var( $query ) ) {
		$msg .= '<p>' . sprintf(
			...
			__( '<strong>Database tables are missing.</strong> This means that MySQL is not running, the application was not installed properly, or someone deleted %s. You really should look at your database now.' ),
			'<code>' . $wpdb->site . '</code>'
		) . '</p>';
	} else {
		$msg .= '<p>' . sprintf(
