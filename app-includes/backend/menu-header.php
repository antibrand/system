 ...
 * @return mixed Returns the output of the menu items.
 */
function app_menu_output( $menu, $submenu, $submenu_as_parent = true ) {
	...
	foreach ( $menu as $key => $item ) {
		...
		if ( ! empty( $item[5] ) ) {
			$id = ' id="' . preg_replace( '|[^a-zA-Z0-9_:.]|', '-', $item[5] ) . '"';
		} else {
			$id = '';
		}
		...
		if ( $is_separator ) {
			...
			if ( ! empty( $menu_hook ) || ( ( 'index.php' != $submenu_items[0][2] ) && file_exists( APP_PLUGINS_PATH . "/$menu_file" ) && ! file_exists( APP_ADMIN_PATH . "/$menu_file" ) ) ) {
				...
				echo "\n\t<a href='{$submenu_items[0][2]}'$class $aria_attributes><div class='wp-menu-image$img_class'$img_style>$img</div><div class='wp-menu-name'>$title</div></a>";
			}
			...
			if ( ! empty( $menu_hook ) || ( ( 'index.php' != $item[2] ) && file_exists( APP_PLUGINS_PATH . "/$menu_file" ) && ! file_exists( APP_ADMIN_PATH . "/$menu_file" ) ) ) {
				...
				echo "\n\t<a href='{$item[2]}'$class $aria_attributes><div class='wp-menu-image$img_class'$img_style>$img</div><div class='wp-menu-name'>{$item[0]}</div></a>";
			}
		}
		...
		if ( ! empty( $submenu_items ) ) {
			...
			foreach ( $submenu_items as $sub_key => $sub_item ) {
				...
				/**
				 * If plugin_page is set the parent must either match the current page or
				 * not physically exist. This allows plugin pages with the same hook to
				...
				if ( ! empty( $menu_hook ) || ( ( 'index.php' != $sub_item[2] ) && file_exists( APP_PLUGINS_PATH . "/$sub_file" ) && ! file_exists( APP_ADMIN_PATH . "/$sub_file" ) ) ) {

					/**
					 * If admin.php is the current page or if the parent exists
					 * as a file in the plugins or admin directory.
					 */
