...
/**
 * Check if rewrite rule already exists in the IIS 7+ configuration file
 *
 * @since 2.8.0
 *
 * @return bool
...
function iis7_rewrite_rule_exists($filename) {
	...
	$rules = $xpath->query('/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'system\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'system\')]');
	if ( $rules->length == 0 )
		return false;
	else
		return true;
...
function iis7_add_rewrite_rule($filename, $rewrite_rule) {
	...
	$system_rules = $xpath->query('/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'system\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'system\')]');
	if ( $system_rules->length > 0 )
		return true;
...
function wp_admin_canonical_url() {
	...
	?>
	<link id="app-admin-canonical" rel="canonical" href="<?php echo esc_url( $filtered_url ); ?>" />
	<script>
		if ( window.history.replaceState ) {
			window.history.replaceState( null, null, document.getElementById( 'app-admin-canonical' ).href + window.location.hash );
		}
	</script>
<?php
}
...
final class WP_Privacy_Policy_Content {
	...
	public static function get_default_content( $descr = false ) {
		...
		$descr && $content .=
			...
			'<p class="privacy-policy-tutorial">' . __( 'By default the application does not collect any personal data about visitors, and only collects the data shown on the User Profile screen from registered users. However some of your plugins may collect personal data. You should add the relevant information below.' ) . '</p>';

		$content .=
			'<h3>' . __( 'Comments' ) . '</h3>';
		$descr && $content .=
			'<p class="privacy-policy-tutorial">' . __( 'In this subsection you should note what information is captured through comments. We have noted the data which the application collects by default.' ) . '</p>';
		$content .=
		...
		$descr && $content .=
			'<p class="privacy-policy-tutorial">' . __( 'By default, the application does not include a contact form. If you use a contact form plugin, use this subsection to note what personal data is captured when someone submits a contact form, and how long you keep it. For example, you may note that you keep contact form submissions for a certain period for customer service purposes, but you do not use the information submitted through them for marketing purposes.' ) . '</p>';

		$content .=
			'<h3>' . __( 'Cookies' ) . '</h3>';
		$descr && $content .=
			'<p class="privacy-policy-tutorial">' . __( 'In this subsection you should list the cookies your web site uses, including those set by your plugins, social media, and analytics. We have provided the cookies which the application installs by default.' ) . '</p>';
		$content .=
		...
		$descr && $content .=
			...
			'<p class="privacy-policy-tutorial">' . __( 'By default the application does not collect any analytics data. However, many web hosting accounts collect some anonymous analytics data. You may also have installed a plugin that provides analytics services. In that case, add information from that plugin here.' ) . '</p>';
		...
		$descr && $content .=
			...
			'<p class="privacy-policy-tutorial">' . __( 'By default the application does not share any personal data with anyone.' ) . '</p>';
	...
	public static function add_suggested_content() {
		...
		wp_add_privacy_policy_content( APP_NAME, $content );
	}
}
